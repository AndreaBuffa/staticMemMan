...
uint32_t memAlloc(void * const handle, const uint32_t size, void * ptr) {
	...
	uint32_t* pBlocks = (uint32_t*)handle + 1 + bitmapLen;
	uint32_t blockIt = 0;
	uint32_t* pNewChunk = pBlocks + wordIt * 32 + offset;
	while (blockIt < size) {
...
uint32_t memFree(void * const handle, const void* ptr) {
	...
	uint32_t idx = 0, found = 0, chunkLen = 0;
	while (idx < totBlocks) {
		if ((uint32_t*)*(pHandle + 1 + bitmapSize + idx) == (uint32_t*)ptr) {
			*(pHandle + 1 + bitmapSize + idx) = 0;
			//1 << idx
			//pHandle + 1 + (idx / 32)
			found = 1;
			chunkLen++;
